...
par(mfrow=c(1,1))
win.graph()
plotawal=ts.plot(y=dataset$BYAN.JK,col="blue",main="Plot Data Saham BYAN.JK",ylab='harga saham')

#UJI STASIONER ADF DATA ASLI
adf.test(dataset$BYAN.JK)         

#DIFFERENCING
dtrans1=as.matrix(diff(log(dataset$BYAN.JK),differences=1))
dtrans2=as.matrix(diff(log(dataset$BYAN.JK),differences=2))
dtrans3=as.matrix(diff(log(dataset$BYAN.JK),differences=3))
dtrans4=as.matrix(diff(log(dataset$BYAN.JK),differences=4))
...
#MODEL ARIMA
arima_terbaik =auto.arima(dataset$BYAN.JK,d=4, seasonal=FALSE,stepwise=FALSE)
...
#ARIMA(0,4,5)-GARCH(1,1) MODEL--------------------------------------------------------------------
garch_spec=ugarchspec(mean.model = list(arima(dataset$BYAN.JK,order=c(0,4,5))),
                      variance.model=list(model="sGARCH"),
                      distribution.model="norm")
garch_fit=ugarchfit(spec=garch_spec,data=dataset$BYAN.JK,solver = 'hybrid')
garch_fit
garch_spec
coef(garch_fit)
