...
par(mfrow=c(1,1))
win.graph()
plotawal=ts.plot(y=dataset$ICBP.JK,col="blue",main="Plot Data Saham ICBP.JK",ylab='harga saham')

#UJI STASIONER ADF DATA ASLI
adf.test(dataset$ICBP.JK)

#DIFFERENCING
dtrans1=as.matrix(diff(log(dataset$ICBP.JK),differences=1))
dtrans2=as.matrix(diff(log(dataset$ICBP.JK),differences=2))
dtrans3=as.matrix(diff(log(dataset$ICBP.JK),differences=3))
dtrans4=as.matrix(diff(log(dataset$ICBP.JK),differences=4))
...
#MODEL ARIMA
arima_terbaik =auto.arima(dataset$ICBP.JK,d=4, seasonal=FALSE,stepwise=FALSE)
...
garch_spec=ugarchspec(mean.model = list(arima(dataset$ICBP.JK,order=c(5,4,0))),
                      variance.model=list(model="sGARCH"),
                      distribution.model="norm")
garch_fit=ugarchfit(spec=garch_spec,data=dataset$ICBP.JK,solver = 'hybrid')
garch_fit
garch_spec
coef(garch_fit)
