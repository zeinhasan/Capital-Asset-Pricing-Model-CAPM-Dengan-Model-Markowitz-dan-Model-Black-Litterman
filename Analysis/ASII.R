...
par(mfrow=c(1,1))
win.graph()
plotawal=ts.plot(y=dataset$ASII.JK,col="blue",main="Plot Data Saham ASII.JK",ylab='harga saham')

#UJI STASIONER ADF DATA ASLI
adf.test(dataset$ASII.JK)         

#DIFFERENCING
dtrans1=as.matrix(diff(log(dataset$ASII.JK),differences=1))
dtrans2=as.matrix(diff(log(dataset$ASII.JK),differences=2))
dtrans3=as.matrix(diff(log(dataset$ASII.JK),differences=3))
dtrans4=as.matrix(diff(log(dataset$ASII.JK),differences=4))
...
#MODEL ARIMA
arima_terbaik =auto.arima(dataset$ASII.JK,d=4, seasonal=FALSE,stepwise=FALSE)
...
#ARIMA(0,4,3)-GARCH(1,1) MODEL--------------------------------------------------------------------
garch_spec=ugarchspec(mean.model = list(arima(dataset$ASII.JK,order=c(0,4,3))),
                      variance.model=list(model="sGARCH"),
                      distribution.model="norm")
garch_fit=ugarchfit(spec=garch_spec,data=dataset$ASII.JK,solver = 'hybrid')
garch_fit
garch_spec
coef(garch_fit)
