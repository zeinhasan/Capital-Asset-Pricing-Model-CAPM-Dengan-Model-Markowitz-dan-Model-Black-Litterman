...
par(mfrow=c(1,1))
win.graph()
plotawal=ts.plot(y=dataset$TLKM.JK,col="blue",main="Plot Data Saham TLKM.JK",ylab='harga saham')

#UJI STASIONER ADF DATA ASLI
adf.test(dataset$TLKM.JK)         

#DIFFERENCING
dtrans1=as.matrix(diff(log(dataset$TLKM.JK),differences=1))
dtrans2=as.matrix(diff(log(dataset$TLKM.JK),differences=2))
dtrans3=as.matrix(diff(log(dataset$TLKM.JK),differences=3))
dtrans4=as.matrix(diff(log(dataset$TLKM.JK),differences=4))
...
#MODEL ARIMA
arima_terbaik =auto.arima(dataset$TLKM.JK,d=4, seasonal=FALSE,stepwise=FALSE)
...
fc_arima#ARIMA(4,4,0)-GARCH(1,1) MODEL--------------------------------------------------------------------
garch_spec=ugarchspec(mean.model = list(arima(dataset$TLKM.JK,order=c(4,4,0))),
                      variance.model=list(model="sGARCH"),
                      distribution.model="norm")
garch_fit=ugarchfit(spec=garch_spec,data=dataset$TLKM.JK,solver = 'hybrid')
garch_fit
garch_spec
coef(garch_fit)
