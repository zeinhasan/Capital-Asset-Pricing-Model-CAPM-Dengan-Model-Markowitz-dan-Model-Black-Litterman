...
par(mfrow=c(1,1))
win.graph()
plotawal=ts.plot(y=dataset$BBNI.JK,col="blue",main="Plot Data Saham BBNI.JK",ylab='harga saham')

#UJI STASIONER ADF DATA ASLI
adf.test(dataset$BBNI.JK)

#DIFFERENCING
dtrans1=as.matrix(diff(log(dataset$BBNI.JK),differences=1))
dtrans2=as.matrix(diff(log(dataset$BBNI.JK),differences=2))
dtrans3=as.matrix(diff(log(dataset$BBNI.JK),differences=3))
dtrans4=as.matrix(diff(log(dataset$BBNI.JK),differences=4))
...
#MODEL ARIMA
arima_terbaik =auto.arima(dataset$BBNI.JK,d=4, seasonal=FALSE,stepwise=FALSE)
...
#ARIMA(0,4,4)-GARCH(1,1) MODEL--------------------------------------------------------------------
garch_spec=ugarchspec(mean.model = list(arima(dataset$BBNI.JK,order=c(0,4,4))),
                      variance.model=list(model="sGARCH"),
                      distribution.model="norm")
garch_fit=ugarchfit(spec=garch_spec,data=dataset$BBNI.JK,solver = 'hybrid')
garch_fit
garch_spec
coef(garch_fit)
